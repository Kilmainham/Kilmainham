		...
		if ( particle && image && image.particles.Count > 0 )
		{
			...
			for ( int i = 0; i < activeparticles.Count; i++ )
			{
				...
				else
				{
					...
					particles[ix].size = Mathf.Lerp(ph.scale[framenum], ph.scale[framenum + 1], subalpha) * scaleall * ps.locscale * sizescale;
					particles[ix].rotation = Mathf.Lerp(ph.rots[framenum][(int)axis], ph.rots[framenum + 1][(int)axis], subalpha) * Mathf.Rad2Deg;
					ix++;
				}
			}
		...
		if ( particle && image && image.optparticles.Count > 0 )
		{
			...
			for ( int i = 0; i < activeparticles.Count; i++ )
			{
				...
				else
				{
					...
					particles[ix].energy = ph.life - ps.time;
					particles[ix].startEnergy = ph.life;
					particles[ix].size = Mathf.Lerp(scl, scl1, subalpha) * scaleall * ps.locscale * sizescale;
